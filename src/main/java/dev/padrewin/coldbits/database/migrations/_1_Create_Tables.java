package dev.padrewin.coldbits.database.migrations;
...
public class _1_Create_Tables extends DataMigration {
    ...
    public void migrate(DatabaseConnector connector, Connection connection, String tablePrefix) throws SQLException {
        ...
        // Check if the old table already exists, if it does then try renaming the table to coldbits_bits and the 'playername' column to 'uuid'
        boolean exists;
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            statement.setString(1, "coldbits");
            exists = statement.executeQuery().next();
        }

        if (exists) {
            try (Statement statement = connection.createStatement()) {
                statement.executeUpdate("ALTER TABLE coldbits RENAME TO " + tablePrefix + "bits");
            } catch (Exception ignored) { }

            try (Statement statement = connection.createStatement()) {
                statement.executeUpdate("ALTER TABLE " + tablePrefix + "bits RENAME COLUMN playername TO uuid");
            } catch (Exception ignored) { }
        } else {
            // Create bits table
            try (Statement statement = connection.createStatement()) {
                statement.executeUpdate("CREATE TABLE " + tablePrefix + "bits (" +
        ...
        if (!dataManager.doesDataExist() && file.exists() && connector instanceof SQLiteConnector) {
            try {
                ...
                ConfigurationSection section = configuration.getConfigurationSection("Bits");
                if (section == null)
